...
SITES = {
    ...
    "freelancer": {
        ...
        # ── 複数通貨の混在 ──────────────────────────────────
        # True にすると、採点プロンプトに為替レートの参照表が差し込まれる。
        # Freelancer.com は1ページの中に USD / INR / CAD / AUD / GBP / EUR が
        # 混在するため、USDに換算しないと予算の大小を比べられない。
        #
        # 【レートの数値はここに書かない】ai_settings（DB）に1か所だけ持つ。
        # サイトが増えても、レートを各サイトのプロンプトへ書き写す必要がない。
        # 数値は半年ごとに見直す運用値なのでDB側（管理画面から変更可）、
        # 「換算が要るサイトかどうか」はサイト追加時に決まる構造なのでコード側、
        # という切り分けにしている。
        #
        # 指定しない（Upwork等）と参照表は差し込まれず、プロンプトは従来と
        # 1文字も変わらない。
        "multi_currency": True,
        "enabled": True,
    },
